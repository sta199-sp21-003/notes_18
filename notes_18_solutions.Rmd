...
ggplot(data = candy_rankings, aes(x = winpercent)) +
  geom_histogram(binwidth = 5, fill = "darkgreen", color = "grey60") +
...
- Slight upward trend: as sugar percentile increases, we see an increase in
  win percentage
- The variability in win percentage as sugar percentile increases is constant
- Weak linear relationship between our variables
...
*The "best" line is the line that minimizes the sum of the squared residuals
...
- Negative residuals for all the points below the fitted line
- Observations that lie directly on the fitted line will have a residual 
  value of 0
...
- Intercept: candies that are in the 0th percentile for sugar content are 
  expected to win 44.6 percent of the time, on average.
    ...
    - We want to think about if the intercept is just a place holder for the
      line or not. If the x values cover 0, then an interpretation will make
      sense.
...
tidy(chocolate_model)
```

$$\widehat{WinPercent} = 42.1 + 18.8~Chocolate$$
...
Here "both" is the baseline level in our `cpa` model.
...
```{r prediction_hard_code}
35.5 + 10.7 * 4
```


```{r prediction_ingredients_model}
new_prediction <- tibble(
  number_ingredients = 4
)

augment(ingredients_model, newdata = new_prediction)
```
...
We should be cautious about making predictions outside the scope of x.

```{r augment}
augment(ingredients_model)
```
...
```{r practice_1_lm}
cost_model <- lm(winpercent ~ pricepercent100, data = candy_rankings)
tidy(cost_model)
```

**Interpretation**: For every one percentage point increase in cost percentile,
we expect, on average, the win percentage to increase by 0.178 percentage 
points. The intercept doesn't have any practical interpretation for this model.
It only serves as a placeholder for the fitted line.
...
```{r practice_2}
tibble(
  pricepercent100 = c(40, 55, 85)
) %>% 
  augment(cost_model, newdata = .)
```
...
```{r practice_3}
caramel_model <- lm(winpercent ~ caramel, data = candy_rankings)
tidy(caramel_model)
```
